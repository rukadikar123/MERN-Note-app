...
function Navbar({userInfo, handleClearSearch, onSearchNote}) {
  ...
  return (
    <>
      <div className="bg-slate-300 flex items-center justify-between w-full px-1 md:px-6 py-2  md:py-4 shadow-md">
        {/* App title */}
        <Link to="/" className="hidden md:block text-slate-500 text-3xl font-medium">
          Notes
        </Link>
        {/* Search Bar */}
        <SearchBar
        ...
        <IoMdMenu size={28} onClick={() => setShowMenu(true)} className="cursor-pointer text-slate-700" />
      </div>
        {/* User Profile & Logout */}
        ...
        <div className="fixed top-0 right-0 h-full md:hidden w-32 bg-white shadow-lg z-50 flex flex-col gap-4 p-4 transition-transform animate-slide-in">
          <div className="flex justify-end">
            <MdClose size={24} onClick={() => setShowMenu(false)} className="cursor-pointer text-slate-700" />
          </div>
          <ProfileInfo userInfo={userInfo} onLogout={onLogout} />
        </div>
      )}
